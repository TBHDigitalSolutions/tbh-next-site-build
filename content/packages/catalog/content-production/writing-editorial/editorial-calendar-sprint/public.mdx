---
# ──────────────────────────────────────────────────────────────────────────────
# IDENTITY & TAXONOMY (Required)
# ──────────────────────────────────────────────────────────────────────────────
id: "content-editorial-calendar-sprint"
slug: "editorial-calendar-sprint"
name: "Editorial Calendar Sprint"
service: "content"
subservice: "writing-editorial"
category: "Writing & Editorial"
tier: "Starter"
badges: ["Popular"]

tags:
  - "editorial"
  - "calendar"
  - "planning"
  - "workflow"
  - "seo"

# ──────────────────────────────────────────────────────────────────────────────
# HERO SECTION (Phase 1)
# ──────────────────────────────────────────────────────────────────────────────
summary: "Ship a tightly prioritized, ready-to-publish 90-day content plan in two weeks—topics, cadence, briefs, and workflow."
description: "A focused planning engagement that turns your goals and inputs (ICP, funnel gaps, keyword themes, product moments) into a 90-day editorial calendar. You get prioritized topics, acceptance criteria, draft briefs, a publishing cadence you can actually keep, and a lean governance model to keep stakeholders aligned."

image:
  src: "/packages/content-production/writing-editorial/editorial-calendar-sprint.png"
  alt: "Editorial calendar example with prioritized topics and schedule"

# ──────────────────────────────────────────────────────────────────────────────
# ICP & PURPOSE (Phase 2)
# ──────────────────────────────────────────────────────────────────────────────
icp: "B2B teams who need a clear, realistic 90-day publishing plan aligned to growth goals."

painPoints:
  - "Campaigns start without a clear POV or success criteria"
  - "Topic ideas aren’t prioritized—everything feels urgent"
  - "Reviews stall because roles/SLAs aren’t defined"
  - "No single source of truth for schedule, links, or status"
  - "Distribution is an afterthought, so reach underperforms"

outcomes:
  - "A 90-day calendar with weekly cadence you can sustain"
  - "Prioritized topic backlog mapped to ICP & funnel stages"
  - "Briefs for the first 4 articles (outline, angle, acceptance criteria)"
  - "Channel plan: blog + distribution notes for SEO/email/social"
  - "Lightweight governance (roles, review SLAs, source-of-truth docs)"

purpose: >
  Translate strategy into a pragmatic, **actually shippable** plan—topics, cadence, briefs, and governance—so
  writers can draft, reviewers can approve, and the calendar keeps moving.

# ──────────────────────────────────────────────────────────────────────────────
# WHAT YOU GET (Phase 3)
# ──────────────────────────────────────────────────────────────────────────────
features:
  - "Prioritized 90-day editorial calendar and topic backlog"
  - "Acceptance criteria per topic aligned to intent and POV"
  - "4 content briefs with outlines, SEO notes, references, and CTA"
  - "Distribution notes per asset for SEO/email/social"
  - "Lean governance model with RACI and review SLAs"
  - "Reusable templates for calendar, briefs, and checklists"

includesGroups:
  - title: "Strategy & Inputs"
    items:
      - "Goal & ICP intake (1 doc + 45-min kickoff)"
      - "Content inventory scan (top 50–100 URLs)"
      - "Keyword/theme alignment (seed list & SERP check)"
      - "Funnel gap assessment (awareness → decision)"
  - title: "Planning & Calendar"
    items:
      - "90-day editorial calendar (week-by-week schedule)"
      - "Topic backlog with priorities & reasons-to-win"
      - "Acceptance criteria per topic (intent, POV, success measures)"
      - "Distribution notes per asset (SEO/email/social)"
  - title: "Briefs & Templates"
    items:
      - "4 content briefs (outline, SEO notes, references, CTA)"
      - "Calendar & brief templates (Google Sheets/Docs)"
      - "Style & voice quick-sheet (1 page)"
  - title: "Workflow & Governance"
    items:
      - "RACI & review SLAs (lightweight)"
      - "Publishing checklist (on-page + QA)"
      - "Source-of-truth doc structure (Drive/Notion ready)"

deliverables:
  - "90-day editorial calendar (week-by-week)"
  - "Topic backlog with priorities and reasons-to-win"
  - "Acceptance criteria per topic (intent, POV, success measures)"
  - "4 content briefs (outline, SEO notes, references, CTA)"
  - "Distribution notes per asset (SEO/email/social)"
  - "Style & voice quick-sheet (1 page)"
  - "Publishing checklist (on-page + QA)"
  - "RACI + review SLAs (lightweight governance)"
  - "Source-of-truth folder structure (Drive/Notion-ready)"
  - "Live handoff session (recording included)"

# ──────────────────────────────────────────────────────────────────────────────
# PRICING (SSOT)
# ──────────────────────────────────────────────────────────────────────────────
price:
  oneTime: 3200
  monthly: null
  currency: "USD"

priceBand:
  tagline: "A production-ready 90-day plan in 10 business days."
  baseNote: "final"
  finePrint: "One brand/locale; up to 2 stakeholders; add-ons available for extra pillars/locales."

# ──────────────────────────────────────────────────────────────────────────────
# DETAILS & TRUST (Phase 4)
# ──────────────────────────────────────────────────────────────────────────────
requirements:
  - "Goals/OKRs, ICP/personas, and product focus for the quarter"
  - "Access to analytics or top content list (GA4/GSC read-only preferred)"
  - "Two sample ‘good’ articles that match desired voice"
  - "One decision-maker for consolidated feedback"

timeline:
  setup: "Day 1–2 — Kickoff, inputs intake, baseline analysis"
  launch: "Day 5 — Draft calendar & backlog review; Day 7–8 — Briefs & governance"
  ongoing: "Day 10 — Final handoff; optional monthly refresh add-on"

ethics:
  - "No AI-generated drafts shipped as final without disclosure"
  - "We avoid derivative ‘keyword-stuffed’ topics that don’t help users"

limits:
  - "Does not include writing full articles (see Blog Article Pack add-on)"
  - "One brand/locale; additional pillars/locales are add-ons"
  - "Max 2 stakeholder reviewers for sprint timelines"

notes: >
  Assumes one brand/locale and up to two stakeholder reviewers. Add-ons available for more pillars/locales,
  more briefs, and tool integrations.

# ──────────────────────────────────────────────────────────────────────────────
# NEXT STEPS (Phase 5)
# ──────────────────────────────────────────────────────────────────────────────
faqs:
  - id: "volume"
    question: "Can we get more than four briefs?"
    answer: "Yes—add additional briefs in packs of 2. We’ll prioritize the highest-impact topics first."
  - id: "cms"
    question: "Will you load the calendar into our PM tool or CMS?"
    answer: "We deliver in Google Sheets/Docs by default. Loading into Asana/Jira/Notion is available as an add-on."
  - id: "seo"
    question: "How deep is the SEO part of this sprint?"
    answer: "We align each topic to search intent and check SERPs for angle/format. Full keyword research or programmatic content planning can be added separately."
  - id: "handoff"
    question: "What happens after the sprint?"
    answer: "You can execute in-house using the calendar, or extend with our Blog Article Pack or Publishing add-ons. We also offer a monthly Calendar Refresh."

addOnRecommendations:
  - "blog-article-pack-4"
  - "cms-publishing"
  - "internal-linking-refresh"

relatedSlugs:
  - "website-copy-refresh"
  - "messaging-playbook"

seo:
  title: "Editorial Calendar Sprint"
  description: "A 2-week engagement to build your 90-day content plan—prioritized topics, briefs, cadence, and workflow."

# ──────────────────────────────────────────────────────────────────────────────
# COPY OVERRIDES (Optional)
# ──────────────────────────────────────────────────────────────────────────────
copy:
  phase1:
    title: "Editorial Calendar Sprint"
    tagline: "A realistic 90-day plan you can actually ship"
  phase2:
    title: "Why this matters"
    tagline: "From wishful plans to a shippable calendar"
  phase3:
    title: "What you get"
    tagline: "Calendar, briefs, governance, and distribution notes"
    includesTitle: "What's included"
    includesSubtitle: "Strategy & Inputs • Planning & Calendar • Briefs & Templates • Workflow & Governance"
    highlightsTitle: "Highlights"
    highlightsTagline: "Prioritized topics + acceptance criteria"
  phase4:
    title: "Details & trust"
    tagline: "Requirements, timelines, ethics, and limits"
  phase5:
    title: "What’s next"
    tagline: "FAQs and recommended add-ons"

# ──────────────────────────────────────────────────────────────────────────────
# EXTRAS (Optional)
# ──────────────────────────────────────────────────────────────────────────────
extras:
  timelineBlocks:
    - title: "Kickoff & Inputs"
      note: "Day 1–2 — goals, ICPs, inventory scan, keyword/theme alignment"
    - title: "Calendar & Backlog"
      note: "Day 3–5 — 90-day schedule, prioritized backlog, acceptance criteria"
    - title: "Briefs & Governance"
      note: "Day 7–8 — 4 briefs, RACI, review SLAs, checklists"
    - title: "Handoff"
      note: "Day 10 — walkthrough, delivery of materials, next steps"
---

## Overview

Editorial cadence slips when plans live in decks and wishful thinking. This sprint converts your quarterly goals, ICPs, and keyword themes into a **90-day, shippable plan** with ownership and acceptance criteria. You’ll leave with a weekly schedule, a prioritized backlog that explains *why these topics win*, four ready-to-draft briefs, and a lean workflow so drafts actually move through review to publish.

We design the plan for the channels you use most—blog as the primary hub, with **distribution notes** for email and social—so every asset has an intentional path to reach. The goal: sustained output that compounds, not a calendar that decays after week two.

## Purpose

Give your team a **single source of truth** for the next 90 days—clear topics, cadence, and acceptance criteria—so writers draft with confidence, reviewers approve quickly, and publishing stays on track. Success looks like a maintained weekly rhythm, fewer stalled drafts, and visible impact from consistent distribution.

## Notes

- One brand/locale and up to two stakeholder reviewers assumed for sprint timing  
- Calendar and briefs delivered in **Google Sheets/Docs** (ready to duplicate)  
- Optional **Monthly Calendar Refresh** keeps backlog prioritized and aligned  
- We avoid derivative, keyword-stuffed ideas in favor of useful topics with a POV  
- Handoff includes recording and a concrete “week one” action plan

## FAQ

### Can we get more than four briefs?
Yes—add additional briefs in packs of two. We’ll tackle the highest-impact topics first.

### Will you load the plan into Asana/Jira/Notion?
By default we deliver Sheets/Docs. Loading into your PM/CMS is available as an add-on.

### How deep is the SEO work here?
Each topic includes **intent alignment** and a **SERP check** for angle/format. Full keyword research or programmatic content planning can be added separately.

### What happens after handoff?
Execute in-house using the calendar and briefs, or add **Blog Article Pack**/**CMS Publishing** support. We also offer a monthly **Calendar Refresh** to keep momentum.

---
